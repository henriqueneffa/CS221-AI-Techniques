...
def create_chain_csp(n: int) -> CSP:
    ...
    #adds variable x(i) with domain of 0 or 1

    for i in range(1,n+1):
        csp.add_variable(f'X{i}',[0,1])

    #factor function for exclusive or
    xorFactorFunction = lambda x,y: x != y

    #finally adding the factors for each binary combination
    for i in range(1,n):
        csp.add_binary_factor(f'X{i}', f'X{i+1}', xorFactorFunction)
...
def create_nqueens_csp(n: int = 8) -> CSP:
    ...
    #let's add one variable for each queen, which represents their actual row given
    #each queen has to be in a row
    for row in range(n):
        csp.add_variable(f'Row_{row}_Queen', list(range(n))) 

    # Defining binary factors (constraints) between all pairs of queens
    for i in range(n):
        for j in range(i+1, n):
            # binary constraint guaranteeing no two queens are in the same column
            csp.add_binary_factor(f'Row_{i}_Queen', f'Row_{j}_Queen', lambda col1,col2: col1 != col2) 


            # binary constraint guaranteeing no two queens are in the same diagonal
            csp.add_binary_factor(f'Row_{i}_Queen', f'Row_{j}_Queen', 
                                  lambda col1,col2,row1=i,row2=j: abs(col1 - col2) != abs(row1 - row2))
...
class BacktrackingSearch:
    ...
    def get_unassigned_variable(self, assignment: Dict):
        ...
        if not self.mcv:
            ...
            # loops over unassigned variables
            for var in self.csp.variables:
                if var not in assignment:
                    # counts number of values for variable that satisfies constraints 
                    count = 0
                    for value in self.domains[var]:
                        if self.satisfies_constraints(assignment,var,value):
                            count += 1

                    # updates most constrainerd
                    if count < smallest_domain_size: 
                        smallest_domain_size = count
...
class SchedulingCSPConstructor:
    ...
    def add_quarter_constraints(self, csp: CSP) -> None:
        ...
        #iterate on student requests on what to take and then on every quarter we'll do scheduling
        for request in self.profile.requests: 
            if request.quarters:
                for quarter in self.profile.quarters: 
                    #if quarter not in the request, add a constraint that this course cannot be taken this q
                    if quarter not in request.quarters: 
                        csp.add_unary_factor((request, quarter), lambda course: course is None)
        ...
        for req in self.profile.requests:
            if req.quarters:
                for qtr in self.profile.quarters:
                    if qtr not in req.quarters:
                        csp.add_unary_factor((req, qtr), lambda course: course is None)
    ...
    def add_unit_constraints(self, csp: CSP) -> None:
        ...
        #loop on each euqarter
        for quarter in self.profile.quarters: 
            unit_variables = []
        #loop through course requests and each course 
            for req in self.profile.requests: 
                for course_id in req.cids:
                    # variable and domain creation
                    unit_var_name = (course_id,quarter)
                    course_info = self.bulletin.courses[course_id]
                    domain_options = [0] + list(range(course_info.minUnits, course_info.maxUnits+1))
                    csp.add_variable(unit_var_name,domain_options)

                    #check if units for course is in valid range to add constraint
                    def constraint(req_course,units):
                        if req_course == course_id: 
                            return (units >= course_info.minUnits ) and (units <= course_info.maxUnits) 
                        return units==0

                    csp.add_binary_factor((req,quarter), unit_var_name, constraint)
                    

                    unit_variables.append(unit_var_name)
            #variable for total number units in the quarter that must be within min and mx limit of quarter
            total_units_var = create_sum_variable(csp, f'total_units_{quarter}', unit_variables, self.profile.maxUnits)
            def check_total_units(total): 
                is_above_min = (total >= self.profile.minUnits)
                is_below_max = (total <= self.profile.maxUnits)
                return (is_above_min and is_below_max)

            csp.add_unary_factor(total_units_var, check_total_units)
