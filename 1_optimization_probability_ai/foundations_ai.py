...
Assignment: HW 1 - Foundations Programming
Instructors: Dorsa Sadigh and Percy Liang
"""
...
def mutate_sentences(sentence: str) -> List[str]:
    ...
    # make sentence become words & extract adjacent pairs
    words = sentence.split()
    pairs = [(words[i], words[i+1]) for i in range(len(words)-1)]

    # recursion to build sentences w/ pairs
    def buildSent(current: List[str], length: int) -> List[List[str]]:
        
        if len(current) == length: return [current]
        ...
        # add sentence if first word matches last
        for pair in pairs:
            if lastW == pair[0]:
                nxtSent.extend(buildSent(current + [pair[1]], length))

        return nxtSent

    # create sentences from each word
    completedSentences = []
    for word in words: completedSentences.extend(buildSent([word], len(words)))

    # words->sentences
    result = set(' '.join(sentence) for sentence in completedSentences)
...
def sparse_vector_dot_product(v1: SparseVector, v2: SparseVector) -> float:
    ...
    """
    # BEGIN_YOUR_CODE (our solution is 1 line of code, but don't worry if you deviate from this)
    # Using a list comprehension to compute products for shared keys in both vectors
    dotProductValues = [v1[k]*v2[k] for k in v1.keys() if k in v2]
    
    return sum(dotProductValues)
    # END_YOUR_CODE
...
def increment_sparse_vector(v1: SparseVector, scale: float, v2: SparseVector,
    ...
    # update v1 based on each v2 value * scale
    for key in v2:
        v1[key] += scale * v2[key]
...
def find_nonsingleton_words(text: str) -> Set[str]:
    ...
    # text to words
    individualWords = text.split()
    
    #count of words
    wordFrequency = collections.defaultdict(int)
    for w in individualWords: wordFrequency[w]+=1

    # count greater than 1
    repeatedWords = {w for w, freq in wordFrequency.items() if freq > 1}
